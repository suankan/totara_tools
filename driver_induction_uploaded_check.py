...
def get_user_completion_history(user, course):
    dbconnection = psycopg2.connect(db_connection_string)
    cursor = dbconnection.cursor()
    cursor.execute('''SELECT timecompleted from mdl_course_completion_history where userid = %s and course = %s''', (get_user_id(user), 107))
    query_result = cursor.fetchone()
    cursor.close()
    dbconnection.close()
        ...
        try:
            if record[4].strip() == 'Completed':
                ...
                if not get_user_id(record[2].strip()):
                    ...
                    timecompleted_db = get_user_completion_from_db(username, 107)
                    print "timecompleted_db:", timecompleted_db
                    if int(new_timecompleted_unixtime) > int(timecompleted_db):
                        print "===Updating the user==="
                        output_csv.writerow([username, 'Driver', 'Driver', new_timecompleted_day, 100])
