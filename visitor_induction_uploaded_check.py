...
def get_course_id(file):
    with open(file, 'r') as f:
        ...
        dbconnection = psycopg2.connect(db_connection_string)
        cursor = dbconnection.cursor()
        cursor.execute('''SELECT id FROM mdl_course where fullname = %s''', (course_name,))
        query_result = cursor.fetchone()
        print "query_result:", query_result
        cursor.close()
        dbconnection.close()
...
def get_user_completion_history(user, course):
    dbconnection = psycopg2.connect(db_connection_string)
    cursor = dbconnection.cursor()
    cursor.execute('''SELECT timecompleted from mdl_course_completion_history where userid = %s and course = %s''', (get_user_id(user), 106))
    query_result = cursor.fetchone()
    cursor.close()
    dbconnection.close()
        ...
        try:
            if record[4].strip() == 'Completed':
                ...
                if not get_user_id(record[2].strip()):
                    ...
                    timecompleted_db = get_user_completion_from_db(username, 106)
                    print "timecompleted_db:", timecompleted_db
                    if int(new_timecompleted_unixtime) > int(timecompleted_db):
                        print "===Updating the user==="
                        output_csv.writerow([username, 'Visitor', 'Visitor', new_timecompleted_day, 100])
